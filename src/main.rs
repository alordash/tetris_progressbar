...
fn gen_shuffled_vec(size: usize) -> Vec<usize> {
...
impl Pendings {
    pub fn new(seed: u64, orders_count: usize, order_size: usize) -> Pendings {
        srand(seed);
        let gen_shuffled_vec = |_| gen_shuffled_vec(order_size);
        let orders: Vec<Vec<usize>> = (0..orders_count).map(gen_shuffled_vec).collect();
    ...
    pub fn update_seed(&mut self, new_seed: u64) {
        self.seed = new_seed;
        srand(self.seed);
        let gen_shuffled_vec = |_| gen_shuffled_vec(self.orders[0].len());
...
fn update_image(img: &mut Image, step: usize, pendings: &Pendings) {
    ...
    let col = (width as f32 - (step as f32 / height as f32).ceil()).max(0.0) as usize;
    for x in (col..width).rev() {
        for y in 0..height {
            let offset = step as i32
                - (width - x - 1) as i32 * height as i32
                - pendings.orders()[x].iter().position(|v| *v == y).unwrap() as i32;
            if offset > 0 {
